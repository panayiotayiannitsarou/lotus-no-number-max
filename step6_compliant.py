...
def _check_size_ok(df: pd.DataFrame, class_col: str, max_per_class: int = MAX_PER_CLASS) -> bool:
    """Ελέγχει ότι κανένα τμήμα δεν υπερβαίνει το max_per_class (default 25)."""
    try:
...
def _rank_candidates(df_before: pd.DataFrame, df_baseline: pd.DataFrame,
                     ...
                     candidates: List, objective: str,
                     max_per_class: int = MAX_PER_CLASS) -> List:
    """
    ...
    for (fromA, classA, fromB, classB, base_reason) in candidates:
        try:
            ...
            if not _check_size_ok(tmp, class_col, max_per_class):
                continue
...
def _commit_best_swap_if_improves(df: pd.DataFrame, df_baseline: pd.DataFrame,
                                  ...
                                  conflict_pairs=None, max_per_class: int = MAX_PER_CLASS) -> Tuple[pd.DataFrame, bool]:
    """
    ...
    ranked = _rank_candidates(df, df_baseline, class_col, gender_col, lang_col, step_col, group_col, candidates, objective, max_per_class)
    if not ranked: 
        return df, False
...
def apply_step6(df: pd.DataFrame,
                *, class_col: str = "ΤΜΗΜΑ", id_col: str = "ID", 
                gender_col: str = "ΦΥΛΟ", lang_col: str = "ΚΑΛΗ_ΓΝΩΣΗ_ΕΛΛΗΝΙΚΩΝ",
                step_col: str = "ΒΗΜΑ_ΤΟΠΟΘΕΤΗΣΗΣ", group_col: str = "GROUP_ID", 
                max_iter: int = MAX_ITER, conflict_pairs: Optional[Any] = None,
                max_class_size: int = MAX_PER_CLASS) -> Dict[str, Any]:
    """
