...
import importlib.util, sys, re, math, numpy as np, pandas as pd
...
def build_step1_6_per_scenario(
    input_excel: str,
    output_excel: str,
    pick_step4: str = "best",
    ...
    """
    max_class_size:
        Ανώτατο θεσμικό όριο μαθητών ανά τμήμα. Προεπιλογή 25 (κυπριακό πλαίσιο),
        παραμετροποιήσιμο ώστε το Lotus να προσαρμόζεται σε σχολεία/εκπαιδευτικά
        συστήματα με διαφορετικό όριο (π.χ. σενάρια σύγκρισης με διεθνή βιβλιογραφία).
    """
    ...
    # STEP 1
    # Ο αριθμός τμημάτων προκύπτει πλέον από το παραμετροποιήσιμο max_class_size,
    # αντί για το προηγούμενο σταθερό όριο 25 (auto-detect μέσω num_classes=None).
    n_students = int(df0.shape[0])
    computed_num_classes = max(2, math.ceil(n_students / max_class_size)) if n_students else None
    df1, _ = m_step1.create_immutable_step1(df0, num_classes=computed_num_classes)
    ...
    with pd.ExcelWriter(output_excel, engine="xlsxwriter") as w:
        for s1col in step1_cols:
            ...
            # STEP 3
            df3, _ = m_h3.apply_step3_on_sheet(
                base.copy(), scenario_col=s2col, num_classes=None,
            ...
            # STEP 5
            df5_tmp, _pen5 = m_step5.step5_place_remaining_students(
                df4.copy(), scenario_col=s4final, num_classes=None,
                max_class_size=max_class_size,
            )
