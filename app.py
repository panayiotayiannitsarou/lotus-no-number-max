...
with colC:
    max_class_size_all = st.number_input(
        "Μέγιστος αριθμός μαθητών/τμήμα",
        min_value=1, max_value=100, value=25, step=1, key="max_class_size_all",
        help="Ανώτατο θεσμικό όριο μαθητών ανά τμήμα. Προεπιλογή 25 (κυπριακό πλαίσιο). "
             "Προσάρμοσε το για σχολεία/συστήματα με διαφορετικό όριο."
    )
with colD:
    if up_all is not None:
        try:
            ...
            min_classes = max(2, math.ceil(N/max_class_size_all)) if N else 0
    ...
    if missing:
        ...
        try:
            ...
            with st.spinner("Τρέχουν τα Βήματα 1→6..."):
                m.build_step1_6_per_scenario(
                    str(input_path), str(step6_path),
                    pick_step4=pick_step4_all,
                    max_class_size=int(max_class_size_all),
                )
